...
// Ensure the global XP window manager instance is available
if (!window.xpSim) {
    // Find the XP window manager instance if not set
    for (const key in window) {
        if (window[key] && window[key].constructor && window[key].constructor.name === 'WindowsXPSimulator') {
            window.xpSim = window[key];
            break;
        }
    }
}

// --- DocViewer: XP-Style Document Viewer with Tabs ---
class DocViewer {
    constructor() {
        this.window = document.getElementById('window-docviewer');
    ...
    bringToFront() {
        // Always use the XP window manager to open/activate the window
        if (window.xpSim && typeof window.xpSim.openWindow === 'function') {
            window.xpSim.openWindow(this.window, 'docviewer');
        } else {
...
// --- Global DocViewer instance ---
window.docViewer = new DocViewer();

// --- Global pending doc for DocViewer ---
window.pendingDocToOpen = null;

// --- Intercept document links to open in DocViewer ---
function setupDocViewerLinks() {
    document.querySelectorAll('a.xp-button, a.docviewer-link').forEach(link => {
        ...
        if (href && /\.(pdf|png|jpg|jpeg|gif|svg)$/i.test(href)) {
            link.addEventListener('click', e => {
                ...
                // Queue the doc to open after window is managed
                window.pendingDocToOpen = { href, name };
                if (window.xpSim && typeof window.xpSim.openWindow === 'function') {
                    window.xpSim.openWindow(document.getElementById('window-docviewer'), 'docviewer');
                }
            });
        }
    });
...
document.addEventListener('DOMContentLoaded', setupDocViewerLinks);
...
// Patch the prototype before creating the instance
(function patchOpenWindowForDocViewer() {
    const proto = WindowsXPSimulator.prototype;
    if (proto._patchedForDocViewer) return; // Only patch once
    const origOpenWindow = proto.openWindow;
    proto.openWindow = function(winElem, winId) {
        if (window._isOpeningWindow) return;
        window._isOpeningWindow = true;
        try {
            origOpenWindow.call(this, winElem, winId);
            if (winId === 'docviewer' && window.pendingDocToOpen && window.docViewer) {
                const { href, name } = window.pendingDocToOpen;
                window.pendingDocToOpen = null;
                window.docViewer.openDoc(href, name, true);
            }
        } finally {
            window._isOpeningWindow = false;
        }
    };
    proto._patchedForDocViewer = true;
})();

// Now create the XP window manager
window.xpSim = new WindowsXPSimulator();
